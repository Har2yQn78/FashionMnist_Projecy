...
# Define the class names for colors
...
def visualize_prediction(img_path, prediction, class_name):
    ...
    plt.title(f'Predicted color: {class_name} (Class: {prediction})')
    plt.axis('off')
    plt.show()

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Predict the color of the clothing.')
    parser.add_argument('--img_path', type=str, required=True, help='Path to the image to be predicted.')
    parser.add_argument('--model_path', type=str, default='../models/my_model_color.keras', help='Path to the saved model file.')
    ...
    if not os.path.exists(args.img_path):
        ...
        print(f'Predicted color: {prediction}, Color name: {class_name}')
